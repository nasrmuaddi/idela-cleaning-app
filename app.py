...
def create_by_question(clean_df: pd.DataFrame, actions: Dict[str, str]) -> pd.DataFrame:
    baseline_cols = find_range_columns(clean_df, BASELINE_START, BASELINE_END)
    out = clean_df[[c for c in META_COLUMNS if c in clean_df.columns]].copy()

    for base_col in baseline_cols:
        action = actions.get(base_col, "No action")
        if action == "drop this question":
            continue

        post_col = f"{base_col}_post"
        if post_col not in clean_df.columns:
            continue

        out[base_col] = clean_df[base_col]
        out[post_col] = clean_df[post_col]

        comp_col = base_col.replace("_mark", "_mark_comparison")
        if comp_col == base_col:
            comp_col = f"{base_col}_comparison"

        out[comp_col] = pd.to_numeric(clean_df[post_col], errors="coerce") - pd.to_numeric(clean_df[base_col], errors="coerce")
...
def apply_actions(df: pd.DataFrame, actions: Dict[str, str]) -> pd.DataFrame:
    ...
    for col, action in actions.items():
        ...
        target_cols = [c for c in [col, post_col] if c in out.columns]

        if action == "change missing to 0":
            for target in target_cols:
                out[target] = out[target].replace({999: 0})
...
if uploaded_file:
    ...
    if len(high_missing) > 0:
        ...
        edited = st.data_editor(
            high_missing_display[["Select Delete", "caseid", "baseline missing %", "endline missing %"] if "caseid" in high_missing_display.columns else ["Select Delete", "baseline missing %", "endline missing %"]],
            use_container_width=True,
            hide_index=False
        )

        delete_indices = edited.index[edited["Select Delete"] == True].tolist()
    else:
        delete_indices = []
    ...
    st.subheader("3) Question Actions")
    ...
    if not baseline_cols:
        ...
        action_df = pd.DataFrame({
            "Question": baseline_cols,
            "Action": ["No action"] * len(baseline_cols)
        })
        ...
        actions = dict(zip(action_df["Question"], action_df["Action"]))
